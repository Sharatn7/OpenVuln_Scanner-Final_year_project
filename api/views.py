...
def  GenerateReport(request):
    ...
    try:
        case_obj = Case.objects.get(case_number=case_number)
        sub = SubDomain.objects.get(case_obj=case_obj)
        dnsenum = DnsEnum.objects.get(case_obj=case_obj)
        osint = WhoIs.objects.get(case_obj=case_obj)
        Nmap = NmapPort.objects.get(case_obj=case_obj)
        osscan = OsScan.objects.get(case_obj=case_obj)
        ssl_certificate = SslCertificate.objects.get(case_obj=case_obj)
        techstack = WappalyzerModel.objects.get(case_obj=case_obj)
        case = Case.objects.get(case_number=case_number)
    except:
        return HTTPResponse("505 Not Found")

    data = {'sub': sub.res, 'dnsenum': dnsenum.res,
            'NmapPort': Nmap.res, 'osint': osint.res, 
            'osscan': osscan.res, 'ssl': ssl_certificate.res,
            'tech': techstack.res,
            'case':case,
            }
    
    pdf = render_to_pdf('user/report.html',data)


    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "Report_%s.pdf" % (case_number)
